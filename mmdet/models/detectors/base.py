...
class BaseDetector(nn.Module, metaclass=ABCMeta):
    ...
    def show_result(self, data, result, dataset=None, score_thr=0.3, outfile=None,size = None,mode=0):
        if isinstance(result, tuple):
            bbox_result, segm_result = result
        else:
            bbox_result, segm_result = result, None

        img_tensor = data['fact_img'][0]
    ...
    def show_result_(self, data, result, dataset=None, score_thr=0.3, outfile=None, size=None,mode=0):
        if isinstance(result, tuple):
            bbox_result, segm_result = result
        else:
            bbox_result, segm_result = result, None

        img_tensor = data['template_img'][0]
        img_metas = data['img_meta'][0].data[0]
        imgs = tensor2imgs(img_tensor, **img_metas[0]['img_norm_cfg'])
        assert len(imgs) == len(img_metas)
